...
test.beforeEach(async () => {
  ...
  AgentStub.findOne.withArgs({
    where: {
      uuid: agentUUID
    }
  }).returns(Promise.resolve(agentFixtures.findByUuid(agentUUID))); //Model CreateMetric stub
  ...
  MetricStub.findByTypeAgentUuid.withArgs(typeAgentUuidArgs).returns(Promise.resolve(metricFixtures.findByTypeAgentUuid(type, agentUUID))); //Modell findByAgentUuid stub

  MetricStub.findByAgentUuid = sandbox.stub();
  MetricStub.findByAgentUuid.withArgs(findByAgentUuidArgs).returns(Promise.resolve(metricFixtures.findByAgentUuid(agentUUID))); // Model findAll Stub
  ...
  MetricStub.findAll.withArgs(metricUuidArgs).returns(Promise.resolve(metricFixtures.findByAgentUuid(agentUUID)));
  MetricStub.findAll.withArgs(typeAgentUuidArgs).returns(Promise.resolve(metricFixtures.findByTypeAgentUuid(type, agentUUID)));
...
test.serial("Metric#create -existing", async t => {
  let metric = await db.Metric.create(agentUUID, newMetric); // console.log(metricFixtures.findByAgentUuid(agentUUID));
...
test.serial("Metric#findByUuid", async t => {
  ...
  t.deepEqual(metric, metricFixtures.findByAgentUuid(agentUUID), "Should be the same.");
...
// });
//# sourceMappingURL=/Users/franndyabreu/Desktop/Platzi-Verse/platziverse-db/node_modules/.cache/ava/7b84e06ae5e64d91011189775420df82.js.map